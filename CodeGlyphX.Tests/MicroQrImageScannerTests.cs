...
public sealed class MicroQrImageScannerTests {
    ...
    [Fact]
    public void EncodedImageFacade_MapsDownscaledGeometryToSourceCoordinates() {
        var code = MicroQrCodeEncoder.EncodeAlphanumeric("SCALED", minVersion: 4, maxVersion: 4);
        var png = MatrixPngRenderer.Render(
            code.Modules,
            new MatrixPngRenderOptions { ModuleSize = 8, QuietZone = 2 });

        Assert.True(MicroQrDecoder.TryDecodeImage(
            png,
            out var decoded,
            out var info,
            new ImageDecodeOptions { MaxDimension = (code.Size + 4) * 4 }));

        Assert.Equal("SCALED", decoded.Text);
        Assert.InRange(info.Geometry.Bounds.X, 14, 18);
        Assert.InRange(info.Geometry.Bounds.Y, 14, 18);
        Assert.InRange(info.Geometry.Bounds.Width, code.Size * 7.5, code.Size * 8.5);
        Assert.InRange(info.Geometry.Bounds.Height, code.Size * 7.5, code.Size * 8.5);
    }
