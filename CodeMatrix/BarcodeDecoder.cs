...
public static class BarcodeDecoder {
    ...
    public static bool TryDecode(byte[] pixels, int width, int height, int stride, PixelFormat format, BarcodeType? expectedType, out BarcodeDecoded decoded) {
        decoded = null!;
        if (!BarcodeScanline.TryGetModules(pixels, width, height, stride, format, out var modules)) return false;
        return TryDecode(modules, expectedType, out decoded);
    }
    ...
    public static bool TryDecode(bool[] modules, BarcodeType? expectedType, out BarcodeDecoded decoded) {
        ...
        if (TryDecodeCode128(trimmed, out var code128)) {
            decoded = new BarcodeDecoded(BarcodeType.Code128, code128);
            return true;
        }
    ...
    private static bool TryDecodeType(BarcodeType type, bool[] modules, out BarcodeDecoded decoded) {
        ...
        switch (type) {
            ...
            case BarcodeType.Code128:
                if (TryDecodeCode128(modules, out var code128)) {
                    decoded = new BarcodeDecoded(BarcodeType.Code128, code128);
                    return true;
                }
                return false;
    ...
    private static bool TryDecodeCode128(bool[] modules, out string text) {
        text = string.Empty;
    ...
    private static bool TryDecodeEan8(bool[] modules, out string text) {
        text = string.Empty;
        if (modules.Length != 67) return false;
    ...
    private static bool TryDecodeEan13(bool[] modules, out string text) {
        text = string.Empty;
        if (modules.Length != 95) return false;
    ...
    private static bool TryDecodeUpcE(bool[] modules, out string text) {
        text = string.Empty;
        if (modules.Length != 51) return false;
        ...
        foreach (var kvp in UpcETables.ParityPatternTable) {
            ...
            if (ParityKey(pattern.NumberSystemZero) == parityKey) {
                text = "0" + new string(digits) + kvp.Key;
                return true;
            }
            if (ParityKey(pattern.NumberSystemOne) == parityKey) {
                text = "1" + new string(digits) + kvp.Key;
                return true;
            }
        }
