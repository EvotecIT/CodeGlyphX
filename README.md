...
| WebP | ✅ | ✅ | Decode: managed VP8L/VP8 + VP8+ALPH (external corpus validation in progress). Animated WebP decodes first frame only. Encode: VP8L lossless subset + animated VP8L; lossy uses managed VP8 intra-only (4x4/16x16) with ALPH for alpha (VP8L fallback on failure). |
