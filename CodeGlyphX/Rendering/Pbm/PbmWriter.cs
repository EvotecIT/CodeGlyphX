...
public static class PbmWriter {
    ...
    private static void WriteRgba32Core(Stream stream, int width, int height, ReadOnlySpan<byte> rgba, int stride, int rowOffset, int rowStride, string bufferName, string bufferMessage) {
        ...
        _ = RenderGuards.EnsureOutputPixels(width, height, "PBM output exceeds size limits.");
        ...
        var rowBytes = RenderGuards.EnsureOutputBytes(((long)width + 7) / 8, "PBM output exceeds size limits.");
        _ = RenderGuards.EnsureOutputBytes((long)height * rowBytes, "PBM output exceeds size limits.");
